...
def extract_articulation_features(audio_file, output_folder):
    start_time = timer()  # Start timer for the file processing

    try:
        phonationf = Articulation()
...
def process_audio(audioFolder, outputFolder):
    ...
    for audio_file in input_folders:
        result, elapsed_time = extract_articulation_features(audio_file, outputFolder)
